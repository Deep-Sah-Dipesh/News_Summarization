...
def find_best_checkpoint():
    log_path = logging_utils.setup_logging(config.V16_MODEL_OUTPUT_DIR, "evaluation_log_v16")
    print(f"--- Starting v16 Evaluation & Save Script ---")
    print(f"Logging all output to: {log_path}\n")
    logging.info("--- Starting Post-Training v16 Evaluation & Save Script ---")
    ...
    checkpoint_dirs = [
        d for d in os.listdir(config.V16_MODEL_OUTPUT_DIR) 
        if d.startswith("checkpoint-") and os.path.isdir(os.path.join(config.V16_MODEL_OUTPUT_DIR, d))
    ]
    
    if not checkpoint_dirs:
        logging.error(f"No checkpoint directories found in {config.V16_MODEL_OUTPUT_DIR}")
        print(f"Error: No checkpoint directories found in {config.V16_MODEL_OUTPUT_DIR}")
        return
    ...
    for chkpt_dir in checkpoint_dirs:
        state_path = os.path.join(config.V16_MODEL_OUTPUT_DIR, chkpt_dir, "trainer_state.json")
        ...
        if eval_log:
            ...
            if metric_value > best_metric_value:
                ...
                best_checkpoint_path = os.path.join(config.V16_MODEL_OUTPUT_DIR, chkpt_dir)
    ...
    summary_header = "\n" + "="*80 + "\n" + "--- FINAL v16 EVALUATION SUMMARY ---".center(80) + "\n"
    ...
    if best_checkpoint_path:
        ...
        logging.info("--- Best v16 model saved successfully! ---")
        print("\n--- Best v16 model saved successfully! ---")
        print("Run 'python test_v16.py' to get final quantitative test scores.")
    else:
